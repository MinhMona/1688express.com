...
namespace NHST.Controllers
{
    public class ConfigurationController
    {
        ...
        public static string Update(int ID, string Websitename, string EmailSupport, string EmailContact, string Hotline, string Address, string Facebook, string Twitter, string GooglePlus, string Instagram,
            ...
            string RegisterMoney,string DaysRejectOrder, string AgentCurrency, string PriceCheckOutWareDefault, string PriceCheckOutWarePerWeight,
            string AccountLocalWeightPrice, int AccountDathangID, string FeeWeChat, string WeightPriceVip, string WeightPriceAgent)
        {
            using (var dbe = new NHSTEntities())
            {
                dbe.Configuration.ValidateOnSaveEnabled = false;
                var conf = dbe.tbl_Configuration.Where(c => c.ID == ID).SingleOrDefault();
                if (conf != null)
                {
                    ...
                    conf.RegisterMoney = RegisterMoney;
                    conf.DaysRejectOrder = DaysRejectOrder;
                    conf.AgentCurrency = AgentCurrency;
                    conf.PriceCheckOutWareDefault = PriceCheckOutWareDefault;
                    conf.PriceCheckOutWarePerWeight = PriceCheckOutWarePerWeight;
                    conf.AccountLocalWeightPrice = AccountLocalWeightPrice;
                    conf.AccountDathangID = AccountDathangID;
                    conf.WeChatFee = FeeWeChat;
                    conf.WeightPriceVip = WeightPriceVip;
                    conf.WeightPriceAgent = WeightPriceAgent;
                    dbe.SaveChanges();
                    return "ok";
                }
                else return null;
        ...
        public static tbl_Configuration GetByTop1()
        {
            using (var dbe = new NHSTEntities())
            {
                var conf = dbe.tbl_Configuration.Where(c => c.ID == 1).SingleOrDefault();
                if (conf != null)
                {
                    return conf;
                }
