...
namespace NHST.Controllers
{
    public class NotificationController
    {
        ...
        public static string Inser(int SenderID, string SenderUsername, int ReceivedID, string ReceivedUsername, int OrderID, string Message, int Status, 
           int NotifType, DateTime CreatedDate, string CreatedBy)
        {
        ...
        public static string UpdateStatus(int ID, int Status, DateTime ModifiedDate, string ModifiedBy)
        {
            using (var dbe = new NHSTEntities())
            {
                var n = dbe.tbl_Notifications.Where(no => no.ID == ID).SingleOrDefault();
                if (n != null)
                {
                    n.Status = Status;
        ...
        public static List<tbl_Notifications> GetByReceivedIDAndTop20(int ReceivedID)
        {
            using (var dbe = new NHSTEntities())
            {
                List<tbl_Notifications> n = new List<tbl_Notifications>();
                n = dbe.tbl_Notifications.Where(no => no.ReceivedID == ReceivedID && no.Status == 1).OrderByDescending(no => no.ID).Take(20).ToList();
                return n;
            }
        }
