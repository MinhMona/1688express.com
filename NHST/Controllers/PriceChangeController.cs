...
namespace NHST.Controllers
{
    public class PriceChangeController
    {
        ...
        public static string Update(int ID, double PriceFromCYN, double PriceToCYN, double Vip0,
             double Vip1, double Vip2, double Vip3, double Vip4, double Vip5, double Vip6, double Vip7, double Vip8, 
             DateTime ModifiedDate, string ModifiedBy)
        {
            using (var dbe = new NHSTEntities())
            {

                var a = dbe.tbl_PriceChange.Where(ac => ac.ID == ID).SingleOrDefault();
                if (a != null)
                {
                    a.PriceFromCYN = PriceFromCYN;
                    a.PriceToCYN = PriceToCYN;
        ...
        public static tbl_PriceChange GetByID(int ID)
        {
            using (var dbe = new NHSTEntities())
            {
                var a = dbe.tbl_PriceChange.Where(f => f.ID == ID).SingleOrDefault();
                if (a != null)
                {
                    return a;
                }
        ...
        public static tbl_PriceChange GetByPriceFT(double price)
        {
            using (var dbe = new NHSTEntities())
            {
                var a = dbe.tbl_PriceChange.Where(f => f.PriceFromCYN < price && f.PriceToCYN >= price).SingleOrDefault();
                if (a != null)
                {
                    return a;
                }
