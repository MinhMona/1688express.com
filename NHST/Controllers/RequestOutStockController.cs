...
namespace NHST.Controllers
{
    ...
    {
        #region CRUD
        public static string Insert(int SmallPackageID, string SmallPackageCode, int MainOrderID, int OrderShopCodeID,
            int Status, DateTime CreatedDate, string CreatedBy, int ExportRequestTurnID)
        {
            using (var dbe = new NHSTEntities())
            {
                ...
                p.OrderShopCodeID = OrderShopCodeID;
                p.Status = Status;
        ...
        public static string InsertT(int SmallPackageID, string SmallPackageCode, int TransportationID, int OrderShopCodeID,
            int Status, DateTime CreatedDate, string CreatedBy, int ExportRequestTurnID)
        {
            using (var dbe = new NHSTEntities())
            {
        ...
        public static string UpdateStatus(int ID, int Status, DateTime ModifiedDate, string ModifiedBy)
        {
            using (var dbe = new NHSTEntities())
            {
                var p = dbe.tbl_RequestOutStock.Where(pa => pa.ID == ID).SingleOrDefault();
                if (p != null)
                {
                    p.Status = Status;
        ...
        public static List<tbl_RequestOutStock> GetAllSQLHelper(string s, int page, int maxrows)
        {
            ...
            while (reader.Read())
            {
                ...
                if (reader["OrderShopCodeID"] != DBNull.Value)
                    entity.OrderShopCodeID = Convert.ToInt32(reader["OrderShopCodeID"]);
        ...
        public static List<RequestOutStoc> GetAllSQLHelper1(string s)
        {
            ...
            var sql = @"SELECT ro.*, sp.OrderTransactionCode, sp.Weight, sp.DateInLasteWareHouse, sp.DateOutWH, er.Note, er.ShippingTypeVNName ";
            ...
            while (reader.Read())
            {
                ...
                if (reader["DateOutWH"] != DBNull.Value)
                    entity.DateExWarehouse = string.Format("{0:dd/MM/yyyy HH:mm}", Convert.ToDateTime(reader["DateOutWH"]));
                int Status = 0;
                if (reader["Status"] != DBNull.Value)
