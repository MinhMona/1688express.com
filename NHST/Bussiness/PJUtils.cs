...
namespace NHST.Bussiness
{
    public class PJUtils
    {
        ...
        public static string ReturnStatusPayHelp(int status)
        {
            if (status == 1)
            {
                return "<span class='bg-red'>Chờ thanh toán</span>";
            }
            else if (status == 2)
            {
            ...
            {
                return "<span class='bg-yellow'>Đã hoàn tiền</span>";
            }
        ...
        public static void ShowMessageBoxHTMLSwAlert(string txt, string type, bool? isRefresh, System.Web.UI.Page page)
        {
            txt = new JavaScriptSerializer().Serialize(txt);
            string p;
            switch (type)
            ...
            JavaScript.AfterPageLoad(page).ExecuteCustomScript("swal({ title: 'Thông báo',text:' " + txt + "',type: '" + p + "', html: true}" + (Convert.ToBoolean(isRefresh.ToString()) ? ", function () { window.location.replace(window.location.href); });" : ");"));
        }
        ...
        public static string IntToRequestAdmin(int i)
        {
            if (i == 0)
                ...
                return "<span class=\"bg-green\">Đang chuyển về kho đích</span>";
            else if (i == 7)
                return "<span class=\"bg-orange\">Đã nhận hàng tại kho đích</span>";
            else if (i == 8)
        ...
        public static string GetTradeType(int TradeType)
        {
            ...
            {
                return "Thanh toán tiền lưu kho";
            }
            else
            {
                return "...";
        ...
        public static string ReturnStatusComplainRequest(int status)
        {
            ...
            {
                return "<span class='bg-orange'>Đã hủy</span>";
            }
        ...
        public static string IntToStringStatusPackage(int status)
        {
            if (status == 0)
                ...
                return "<span class=\"bg-red\">Chưa về kho TQ</span>";
            else if (status == 2)
                return "<span class=\"bg-orange\">Đã về kho TQ</span>";
            else if (status == 3)
                return "<span class=\"bg-green\">Đã về kho đích</span>";
            else 
                return "<span class=\"bg-blue\">Đã giao khách hàng</span>";
        }
        public static string GeneralTransportationOrderNewStatus(int status)
        {
            if (status == 0)
                ...
                return "<span class=\"bg-bronze\">Đã thanh toán</span>";            
            else
                return "<span class=\"bg-blue\">Đã giao khách hàng</span>";
        }
